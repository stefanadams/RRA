...
#_______________________________________________________________________________
	if($_POST['changePassword'] && $_SESSION['resetUsername']!=$adminConfig['adminUsername']){
		...
		sql("update membership_users set passMD5='".md5($_POST['newPassword'])."' where memberID='".$_SESSION['resetUsername']."'");
...
#_______________________________________________________________________________
	if($_GET['key']){
		...
		if($_GET['key']==$_SESSION['resetKey'] &&  $_SESSION['resetUsername']!=$adminConfig['adminUsername']){
			$res=sql("select * from membership_users where memberID='".$_SESSION['resetUsername']."'");
...
#_______________________________________________________________________________
	if($_POST['reset']){
		...
		$res=sql("select * from membership_users where memberID='$username' or email='$email' limit 1");
		if($row=mysql_fetch_assoc($res)){
