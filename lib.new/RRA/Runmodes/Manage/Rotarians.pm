...
sub rotarians_POST : StartRunmode RequireAjax Authen Authz(':admins') {
	...
	my ($sql, @bind) = sql_interp 'SELECT count(*) FROM manage_rotarians_vw WHERE', ($sField&&$sOper{$sOper}? (sql($sField).$sOper{$sOper},\$sValue) : ('1=1'));
	my $records = $self->dbh->selectrow_array($sql, {}, @bind);
	my $pages = $records > 0 ? int(($records / $rows) + 0.99) : 0;
	my $start = $page * $rows - $rows || 0;
	($sql, @bind) = sql_interp 'SELECT * FROM manage_rotarians_vw WHERE', ($sField&&$sOper{$sOper}? (\$sField,$sOper{$sOper},\$sValue) : ('1=1')), 'ORDER BY', sql($sidx), sql($sord), 'LIMIT', \$rows, 'OFFSET', \$start;
