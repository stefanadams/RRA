<?php
	...
	if($_POST['saveChanges']==''){
		...
		$recipient=($sendToAll ? "All groups" : ($isGroup ? sqlValue("select name from membership_groups where groupID='$groupID'") : sqlValue("select memberID from membership_users where memberID='$memberID'")));
		if(!$recipient){
			?>
			<div class="status">
	...
	// begin sending emails
		...
		if($sendToAll){
			...
			$res=sql("select email from membership_users where memberID='$memberID'");
		}
		...
		if(!$fp=fopen(dirname(__FILE__)."/$queueFile.php", "w")){
			?>
			<div class="status">
