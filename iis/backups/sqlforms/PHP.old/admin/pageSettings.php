<?php
	...
	if($_POST['saveChanges']!=''){
		// validate inputs
		$adminUsername=makeSafe($_POST['adminUsername']);

		// check if this member already exists

		if($adminConfig['adminUsername']!=$adminUsername && sqlValue("select count(1) from membership_users where memberID='$adminUsername'")){
			// display status
		...
		if(!$fp=@fopen($conFile, "w")){
			...
			// update admin member
			sql("update membership_users set memberID='$adminUsername', passMD5='$adminPassword', email='$senderEmail', comments=concat(comments, '\\n', 'Record updated automatically on ".date('Y-m-d')."') where memberID='".$adminConfig['adminUsername']."'");
		}
