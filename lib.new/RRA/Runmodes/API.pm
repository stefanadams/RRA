...
sub bidding_GET : Runmode RequireAjax {
	my $self = shift;
	return $self->sql_interp('SELECT item_id,item_id id,number,itemurl,item,description,auctioneer,value,highbid,startbid,minbid,highbidder,bellringer,timerminutes timer,donor,donorurl,advertisement,advertisement message FROM items_current_vw WHERE (status="Bidding" OR status="Sold") ORDER BY number')->selectall_arrayref->to_json;
}
