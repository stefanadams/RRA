...
sub tooltip_GET : Runmode RequireAjax {
	my $self = shift;
        my ($sql, @bind) = sql_interp 'SELECT item_id,item_id id,number,itemurl,item,description,donor,donorurl,advertisement,advertisement message FROM items_current_vw WHERE',{item_id=>$self->param('item_id')};
...
sub return_json {
		...
		my $photodir=$self->cfg('PHOTOS');
		$row->{img} =~ s/^$photodir/photos\//;
