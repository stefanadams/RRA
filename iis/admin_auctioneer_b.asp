...
<%
		SQLStatement	= "SELECT id, name, status, sellername, suggestedprice, bid, bidderID, timer, timeradmin, advertisement, active, moreinfo FROM items WHERE (status = 'Bidding' Or status = 'Halt') And Auctioneer = 1 Order By SequenceNumber"
		...
		if objRS("timeradmin") = "end" Then
			strTimer =  strTimer & "<br><a href='admin_stop_auction_02.asp?id=" & objRS("id") & "&x=b'><font face='arial' size='1' color='blue'><nobr>End Auction</nobr></font></a>"
			trcolor = "#F2F2E0"
		end if
		...
		if objRS("timeradmin") = "Halt" Then
			strTimer =  "<a href='admin_stop_auction_03.asp?id=" & objRS("id") & "&x=b'><font face='arial' size='1' color='blue'><nobr>Clear</nobr></font></a>"
			trcolor = "#87cefa"
		end if
		...
		Response.Write " valign='top' style='border-left: 1 solid #808080; border-right: 1 solid #808080; border-bottom: 1 solid #808080' align='center'>"
			if moreinfo <> "" Then Response.Write "<a href='" & moreinfo & "' target='_new'><b>More Info</b></a> | "
			if newbidcolor <> "" Then Response.Write "<a href='clear_bid_notice.asp?who=b&id=" & objRS("id") & "'>"
...
'-----------------
		...
		'end of row
                ...
                if CInt(b) < 8 then
                        Response.Write "<font size='1'><a href='admin_start_auction_03.asp?id=" & objRS("id") & "&auctioneer=b'>Start Now</a></font>"
                else
                        Response.Write "&nbsp;"
                end if
                Response.Write "</td>"
