<?php
	...
	*/
	...
	$sourceMemberID=makeSafe($_GET['sourceMemberID']);
	$destinationGroupID=intval($_GET['destinationGroupID']);
	$destinationMemberID=makeSafe($_GET['destinationMemberID']);
	...
	if($sourceGroupID && $sourceMemberID && $destinationGroupID && ($destinationMemberID || $moveMembers) && $_GET['beginTransfer']!=''){
		...
		*/
		if(!sqlValue("select count(1) from membership_users where memberID='$sourceMemberID' and groupID='$sourceGroupID'")){
			if($sourceMemberID!=-1){
		...
		if(!$moveMembers){
			if(!sqlValue("select count(1) from membership_users where memberID='$destinationMemberID' and groupID='$destinationGroupID'")){
		...
		if($moveMembers && $sourceMemberID!=-1){
			...
			sql("update membership_users set groupID='$destinationGroupID' where memberID='$sourceMemberID' and groupID='$sourceGroupID'");
			$newGroup=sqlValue("select name from membership_users u, membership_groups g where u.groupID=g.groupID and u.memberID='$sourceMemberID'");

			// change group of source member's data
			sql("update membership_userrecords set groupID='$destinationGroupID' where memberID='$sourceMemberID' and groupID='$sourceGroupID'");
			$dataRecs=sqlValue("select count(1) from membership_userrecords where memberID='$sourceMemberID' and groupID='$destinationGroupID'");
			...
			$srcDataRecsBef=sqlValue("select count(1) from membership_userrecords where memberID='$sourceMemberID' and groupID='$sourceGroupID'");
			sql("update membership_userrecords set groupID='$destinationGroupID', memberID='$destinationMemberID' where memberID='$sourceMemberID' and groupID='$sourceGroupID'");
			$srcDataRecsAft=sqlValue("select count(1) from membership_userrecords where memberID='$sourceMemberID' and groupID='$sourceGroupID'");
			...
			// change group of source member's data
			$recsBef=sqlValue("select count(1) from membership_userrecords where memberID='$destinationMemberID'");
			...
			$recsAft=sqlValue("select count(1) from membership_userrecords where memberID='$destinationMemberID'");
			...
			<tr>
				...
				<td class="tdCell">
					<?php
						...
						if($res=sql("select memberID, memberID from membership_users where groupID='$sourceGroupID' order by memberID")){
	...
	// STEP 4, source group same as destination
		if($destinationGroupID && $destinationGroupID==$sourceGroupID){
			...
			<tr>
				...
				<td class="tdCell">
					<?php
						echo htmlSQLSelect("destinationMemberID", "select memberID, memberID from membership_users where groupID='$destinationGroupID' and memberID!='$sourceMemberID' order by memberID", $destinationMemberID);
					?>
					</td>
				</tr>
			<tr>
				...
				if(sqlValue("select count(1) from membership_users where groupID='$destinationGroupID'")>0){
					...
					<tr>
						<td class="tdCell" colspan="2">
							...
							<?php
								echo htmlSQLSelect("destinationMemberID", "select memberID, memberID from membership_users where groupID='$destinationGroupID' order by memberID", $destinationMemberID);
							?>
							</td>
						</tr>
					<?php
