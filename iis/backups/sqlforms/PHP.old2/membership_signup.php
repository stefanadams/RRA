<?php
	...
	if($_POST['signUp']!=''){
		...
		if($adminConfig['notifyAdminNewMembers']==2 && !$needsApproval){
			@mail($adminConfig['senderEmail'], '[washingtonrotary_master] New member signup', "A new member has signed up for Northwind.\n\nMember name: $memberID\nMember group: ".sqlValue("select name from membership_groups where groupID='$groupID'"));
		}elseif($adminConfig['notifyAdminNewMembers']>=1 && $needsApproval){
			@mail($adminConfig['senderEmail'], '[washingtonrotary_master] New member waiting approval', "A new member has signed up for Northwind.\n\nMember name: $memberID\nMember group: ".sqlValue("select name from membership_groups where groupID='$groupID'"));
		}

		// redirect to thanks page
		redirect("membership_thankyou.php");
...
<center>
<h1 class="TableTitle"><?php echo $Translation['sign up here']; ?></h1>
	...
	<table>
		...
		<tr>
			...
			<td align="left" class="TableBody" width="400">
				<?php
					echo htmlSQLSelect('groupID', "select groupID, concat(name, if(needsApproval=1, ' *', ' ')) from membership_groups where allowSignup=1 order by name", $groupID);
				?>
