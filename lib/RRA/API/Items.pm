...
sub add_POST : Runmode {
	...
	$self->param('year', sql('auction_year()'));
	$self->param('number', sql('NextItemNumber()'));
	...
	$self->dbh->do($sql, {}, @bind) or return $self->to_json({sc=>'false',msg=>"Error: ".$self->dbh->errstr});
	return $self->to_json({sc=>'true',msg=>"",number=>$self->dbh->selectrow_array("SELECT number FROM items WHERE id=LAST_INSERT_ID()")});
}
...
sub items_POST : StartRunmode { #Authen Authz('admins') {
	...
	my ($sql, @bind) = ("SELECT item_id, year, number, category, item, description, value, itemurl, donor_id, advertisement, if(donor_id,concat_ws(':', donor,donor_id),null) donor, if(stockitem_id,concat_ws(':', item,stockitem_id),null) stockitem FROM items_vw $search ORDER BY $sidx $sord LIMIT ?,?", ($sField && $sOper{$sOper} && $sValue ? $sValue : ()), $start, $rows);
