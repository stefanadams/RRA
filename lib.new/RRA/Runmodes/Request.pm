...
sub approve_POST : Runmode Authen Authz(':operators') {
	my $self = shift;
	my ($sql, @bind) = $self->sql_interp('SELECT request_id,request,value,approve,id session_id FROM requests JOIN sessions on requests.session_id=sessions.id WHERE',{request_id=>$self->param('request_id')});
		...
		case 'newbid' {
			...
			if ( $self->param('approve') ) {
				...
				unless ( $bidder_id ) {
					...
					my ($sql, @bind) = $self->sql_interp('SELECT bidder_id FROM bidders WHERE',{year=>2012,phone=>$self->param('phone')});
				...
				my ($sql, @bind) = $self->sql_interp('INSERT INTO bids',{bidder_id=>$bidder_id,item_id=>$item_id,bid=>$bid});
			...
			my ($sql, @bind) = $self->sql_interp('INSERT INTO alerts',{alert=>$request->{session_id},msg=>$msg},'ON DUPLICATE KEY UPDATE',{msg=>$msg});
			$self->dbh->do($sql, {}, @bind) or return $self->to_json({bid=>undef,msg=>"Error placing bid"});
		}
		case 'newuser' {
			...
			if ( $self->param('approve') ) {
				...
				my ($sql, @bind) = $self->sql_interp('INSERT INTO bidders',{username=>$username,phone=>$phone,bidder=>$bidder});
			...
			my ($sql, @bind) = $self->sql_interp('INSERT INTO alerts',{alert=>$request->{session_id},msg=>$self->param('approve')?"Registered!  :D":"Rejected.  :("},'ON DUPLICATE KEY UPDATE',{msg=>$msg});
