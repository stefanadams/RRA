<?php
	...
	$anonMemberID=$adminConfig['anonymousMember'];

	// request to save changes?
	if($_POST['saveChanges']!=''){
		// validate data
		$oldMemberID=makeSafe($_POST['oldMemberID']);
		$memberID=makeSafe($_POST['memberID']);
		...
		if($oldMemberID==''){ // new member
			...
			if(sqlValue("select count(1) from membership_users where memberID='$memberID'")){
			...
			// make sure member name is unique
			if($oldMemberID!=$memberID && sqlValue("select count(1) from membership_users where memberID='$memberID'")){
				echo "<div class=\"error\">Error: Username already exists. You must choose a unique username.</div>";
				include("./incFooter.php");
			}
			...
			$oldIsApproved=sqlValue("select isApproved from membership_users where memberID='$memberID'");

			// update member
			$upQry="UPDATE `membership_users` set memberID='$memberID', passMD5=".($password!='' ? "'".md5($password)."'" : "passMD5").", email='$email', groupID='$groupID', isBanned='$isBanned', isApproved='$isApproved', custom1='$custom1', custom2='$custom2', custom3='$custom3', custom4='$custom4', comments='$comments' WHERE memberID='$oldMemberID'";
			sql($upQry);

			// if memberID was changed, update membership_userrecords
			if($oldMemberID!=$memberID){
				sql("update membership_userrecords set memberID='$memberID' where memberID='$oldMemberID'");
			}
	...
	if($memberID!=''){
		...
		$res=sql("select * from membership_users where memberID='$memberID'");
...
<?php }elseif($memberID==$adminConfig['adminUsername']){ ?>
...
<form method="post" action="pageEditMember.php" onSubmit="return jsValidateMember();">
	...
	<?php if($memberID!=$adminConfig['adminUsername']){ ?>
		<tr>
